"""
FastAPI wrapper for the LangGraph chat agent
...
from agent.langgraph_workflow import handle_chat, get_chat_history, handle_chat_streaming
...
app = FastAPI(
    title="AI Business Advisor API",
    description="REST API for the AI Business Advisor chat agent",
    version="1.0.0"
)
...
async def health_check():
    """Health check endpoint"""
    return {"status": "healthy", "service": "AI Business Advisor API"}
...
async def root():
    ...
    return {
        "message": "AI Business Advisor API",
        "version": "1.0.0",
        "endpoints": {
            "health": "/health",
...
async def stream_chat(request: ChatMessage):
    ...
    async def event_stream() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async for chunk in handle_chat_streaming(request.message, session_id):
...
async def stream_chat(chat_request: ChatMessage):
    ...
    async def event_stream():
        ...
        if not chat_request.message.strip():
            yield f"data: {json.dumps({'error': 'Message cannot be empty', 'session_id': session_id})}\n\n"
            return
        ...
        try:
            ...
            from agent.langgraph_workflow import handle_chat_streaming
            
            # Stream the response word by word
            async for chunk in handle_chat_streaming(chat_request.message, session_id):
                yield f"data: {json.dumps(chunk)}\n\n"
